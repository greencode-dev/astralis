            ...
            <form method="POST" action="{{ route('admin.missioni.update', $missione) }}" enctype="multipart/form-data">
                ...
                <div class="mb-5 mt-5">
                    ...
                    <textarea name="descrizione" id="descrizione" rows="5"
                              class="admin-input">{{ old('descrizione', $missione->descrizione) }}</textarea>
                    @error('descrizione')<p class="mt-1 text-sm text-red-500">{{ $message }}</p>@enderror
                </div>

                <div class="flex items-center gap-3">
                    <button type="submit"
                             class="px-6 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 bg-admin-primary text-admin-bg hover:brightness-90">Aggiorna Missione</button>

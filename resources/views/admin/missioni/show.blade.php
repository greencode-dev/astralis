...
@section('content')
    <div class="max-w-4xl">
        <a href="{{ route('admin.missioni.index') }}" class="inline-flex items-center gap-2 text-sm mb-6 transition-colors duration-150 hover:text-[#22D3EE]" style="color: #9CA3AF;">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="rounded-xl p-6 flex flex-col items-center text-center" style="background-color: #111128; border: 1px solid rgba(34, 211, 238, 0.1);">
                @if ($missione->logo)
                    <img loading="lazy" src="{{ Storage::url('missioni/' . $missione->logo) }}" alt="{{ $missione->nome }}" class="w-24 h-24 rounded-xl object-cover mb-4" style="border: 2px solid rgba(34, 211, 238, 0.2);">
                @else
                    <div class="w-24 h-24 rounded-xl flex items-center justify-center text-3xl mb-4" style="background-color: rgba(34, 211, 238, 0.1); color: #22D3EE;" role="img" aria-label="{{ $missione->nome }}">
            ...
            <div class="rounded-xl p-4 text-center" style="background-color: #111128; border: 1px solid rgba(34, 211, 238, 0.1);">
                ...
                <p class="text-sm font-medium" style="color: #F0F0FA;">
                    @if ($missione->sito_web)
                        <a href="{{ $missione->sito_web }}" target="_blank" rel="noopener" class="transition-colors duration-150 hover:text-[#A855F7]" style="color: #22D3EE;">Visita →</a>
                    @else
                        —
                    @endif
                </p>
                        ...
                        <tbody>
                            @foreach ($missione->corpiCelesti as $corpo)
                                <tr style="border-bottom: 1px solid rgba(34, 211, 238, 0.05);">
                                    <td class="py-3 px-4">
                                        <a href="{{ route('admin.corpi-celesti.show', $corpo) }}" class="transition-colors duration-150 hover:text-[#22D3EE]" style="color: #F0F0FA;">{{ $corpo->nome }}</a>
                                    </td>
                                    <td class="py-3 px-4">
                                        @if ($corpo->categoria)

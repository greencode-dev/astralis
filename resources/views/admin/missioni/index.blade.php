            ...
            <tbody>
                @forelse ($missioni as $missione)
                    <tr style="border-bottom: 1px solid rgba(34, 211, 238, 0.05);" class="transition-colors duration-150" onmouseover="this.style.backgroundColor='rgba(34,211,238,0.03)';" onmouseout="this.style.backgroundColor='transparent';">
                        <td class="py-3 px-4">
                            @if ($missione->logo)
                                <img loading="lazy" src="{{ Storage::url('missioni/' . $missione->logo) }}" alt="{{ $missione->nome }}" class="w-8 h-8 rounded-lg object-cover" style="border: 1px solid rgba(34, 211, 238, 0.2);">
                            @else
                                <div class="w-8 h-8 rounded-lg flex items-center justify-center text-sm" style="background-color: rgba(34, 211, 238, 0.1); color: #22D3EE;" role="img" aria-label="{{ $missione->nome }}">
                        ...
                        <td class="py-3 px-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.missioni.show', $missione) }}" class="p-2 rounded-lg transition-all duration-200" style="color: #9CA3AF;" onmouseover="this.style.color='#22D3EE'; this.style.backgroundColor='rgba(34,211,238,0.1)';" onmouseout="this.style.color='#9CA3AF'; this.style.backgroundColor='transparent';" aria-label="Visualizza {{ $missione->nome }}" title="Visualizza">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                </a>
                                <a href="{{ route('admin.missioni.edit', $missione) }}" class="p-2 rounded-lg transition-all duration-200" style="color: #9CA3AF;" onmouseover="this.style.color='#F97316'; this.style.backgroundColor='rgba(249,115,22,0.1)';" onmouseout="this.style.color='#9CA3AF'; this.style.backgroundColor='transparent';" aria-label="Modifica {{ $missione->nome }}" title="Modifica">
                                ...
                                <form method="POST" action="{{ route('admin.missioni.destroy', $missione) }}" class="inline" onsubmit="return confirm('Sei sicuro di voler eliminare {{ $missione->nome }}?');">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="p-2 rounded-lg transition-all duration-200" style="color: #9CA3AF;" onmouseover="this.style.color='#EF4444'; this.style.backgroundColor='rgba(239,68,68,0.1)';" onmouseout="this.style.color='#9CA3AF'; this.style.backgroundColor='transparent';" aria-label="Elimina {{ $missione->nome }}" title="Elimina">

            ...
            <tbody>
                @forelse ($missioni as $missione)
                    <tr style="border-bottom: 1px solid rgba(34, 211, 238, 0.05);" class="hover:bg-[rgba(34,211,238,0.03)]">
                        ...
                        <td class="py-3 px-4">
                            <a href="{{ route('admin.missioni.show', $missione) }}" class="font-medium transition-colors duration-150 hover:text-[#22D3EE]" style="color: #F0F0FA;">{{ $missione->nome }}</a>
                        </td>
                        ...
                        <td class="py-3 px-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.missioni.show', $missione) }}" class="p-2 rounded-lg transition-all duration-200 hover:text-[#22D3EE] hover:bg-[rgba(34,211,238,0.1)]" style="color: #9CA3AF;" aria-label="Visualizza {{ $missione->nome }}" title="Visualizza">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                </a>
                                <a href="{{ route('admin.missioni.edit', $missione) }}" class="p-2 rounded-lg transition-all duration-200 hover:text-[#F97316] hover:bg-[rgba(249,115,22,0.1)]" style="color: #9CA3AF;" aria-label="Modifica {{ $missione->nome }}" title="Modifica">
                                ...
                                <form method="POST" action="{{ route('admin.missioni.destroy', $missione) }}" class="inline" onsubmit="return confirm('Sei sicuro di voler eliminare {{ $missione->nome }}?');">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="p-2 rounded-lg transition-all duration-200 hover:text-[#EF4444] hover:bg-[rgba(239,68,68,0.1)]" style="color: #9CA3AF;" aria-label="Elimina {{ $missione->nome }}" title="Elimina">
                ...
                @empty
                    <tr>
                        <td colspan="6" class="py-12 text-center">
                            ...
                            <a href="{{ route('admin.missioni.create') }}" class="text-sm font-medium transition-colors duration-150 hover:text-[#A855F7]" style="color: #22D3EE;">Crea la prima missione</a>
                        </td>
                    </tr>
                @endforelse
            </tbody>

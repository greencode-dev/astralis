    ...
    <div class="max-w-2xl">
        ...
        <div class="rounded-xl p-6" style="background-color: #111128; border: 1px solid rgba(34, 211, 238, 0.1);">
            <form method="POST" action="{{ route('admin.galleria.store') }}" enctype="multipart/form-data">
                ...
                <div class="mb-5">
                    <label for="percorso" class="block text-sm font-medium mb-2" style="color: #F0F0FA;">Immagine <span style="color: #EF4444;">*</span></label>
                    <input type="file" name="percorso" id="percorso" accept="image/jpeg,image/png,image/webp" required
                           ...
                           onfocus="this.style.borderColor='#22D3EE'; this.style.boxShadow='0 0 0 3px rgba(34,211,238,0.1)';"
                           onblur="this.style.borderColor='rgba(34,211,238,0.2)'; this.style.boxShadow='none';">
                    <p class="mt-1 text-xs" style="color: #6B7280;">Max 2MB. Formati: JPG, PNG, WebP. Verrà ridimensionata a 1200px.</p>
                    @error('percorso')
                        <p class="mt-1 text-sm" style="color: #EF4444;">{{ $message }}</p>
                    @enderror
                </div>
